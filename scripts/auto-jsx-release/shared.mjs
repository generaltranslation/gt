...
export const releaseBranch = 'e/release/auto-jsx-alpha';
...
export function requireExecutionContext(env, expectedSha) {
  requireCondition(
    env.GITHUB_REPOSITORY === 'generaltranslation/gt' &&
      env.GITHUB_EVENT_NAME === 'workflow_dispatch' &&
      env.GITHUB_REF === `refs/heads/${releaseBranch}` &&
      env.GITHUB_SHA === expectedSha &&
      env.ACTIONS_ID_TOKEN_REQUEST_TOKEN &&
      env.ACTIONS_ID_TOKEN_REQUEST_URL,
    'Publishing requires an explicit GitHub workflow dispatch with matching SHA and OIDC'
  );
}
