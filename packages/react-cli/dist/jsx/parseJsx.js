...
const addGTIdentifierToSyntaxTree_1 = __importDefault(
  require('../data-_gt/addGTIdentifierToSyntaxTree')
);
...
function buildJSXTree(node, unwrappedExpressions, updates, errors, file) {
  if (t.isJSXExpressionContainer(node)) {
    ...
    const elementIsVariable = VARIABLE_COMPONENTS.includes(
      typeName !== null && typeName !== void 0 ? typeName : ''
    );
    const props = {};
          ...
          if (t.isStringLiteral(attr.value)) {
            ...
            if (
              ...
              if (
                ...
                !(0, evaluateJsx_1.isStaticExpression)(attr.value.expression)
                  .isStatic
              ) {
                unwrappedExpressions.push(
                  (0, generator_1.default)(attr.value).code
                );
              }
            }
...
function parseJSXElement(node, updates, errors, file) {
  ...
  if (name.type === 'JSXIdentifier' && name.name === 'T') {
        ...
        // If it's an expression container like id={"hello"}, id={someVar}, etc.
          ...
          if (attrName === 'id' || attrName === 'context') {
            ...
            if (!staticAnalysis.isStatic) {
              errors.push(
                (0, warnings_1.warnVariableProp)(file, attrName, code)
              );
            }
          }
    ...
    const whitespaceHandledTree = (0,
    trimJsxStringChildren_1.handleChildrenWhitespace)(initialTree);
    const tree = (0, addGTIdentifierToSyntaxTree_1.default)(
    ...
    if (unwrappedExpressions.length > 0) {
      errors.push(
        (0, warnings_1.warnHasUnwrappedExpression)(
          file,
          id,
          unwrappedExpressions
        )
