...
const createESBuildConfig_1 = __importDefault(
  require('./config/createESBuildConfig')
);
...
class BaseCLI {
  ...
  setupTranslateCommand() {
    commander_1.program
      ...
      .option(
        ...
        (0, findFilepath_1.default)(['./tsconfig.json', './jsconfig.json'])
      )
      .option(
        '--dictionary <path>',
        'Path to dictionary file',
        (0, findFilepath_1.default)([
          './dictionary.js',
          './src/dictionary.js',
          './dictionary.json',
          './src/dictionary.json',
      ...
      .option(
        ...
        (0, findFilepath_1.findFilepaths)([
          './src',
          './app',
          './pages',
          './components',
  ...
  handleSetupCommand(options) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const { errors, filesUpdated, warnings } =
        yield this.scanForContent(options);
      if (errors.length > 0) {
      ...
      if (filesUpdated.length > 0) {
        console.log(
          filesUpdated
            .map((file) => `${chalk_1.default.green('-')} ${file}`)
            .join('\n')
        );
        console.log();
        console.log(
          chalk_1.default.green('Please verify the changes before committing.')
        );
      }
  ...
  handleTranslateCommand(initOptions) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (!options.baseUrl) options.baseUrl = internal_1.defaultBaseUrl;
      ...
      if (
        ...
        !(0, generaltranslation_1.isValidLocale)(options.defaultLocale)
      )
        throw new Error(
      ...
      if (options.locales) {
        for (const locale of options.locales) {
          if (!(0, generaltranslation_1.isValidLocale)(locale)) {
            throw new Error(
              `locales: "${(_a = options === null || options === void 0 ? void 0 : options.locales) === null || _a === void 0 ? void 0 : _a.join()}", ${locale} is not a valid locale!`
            );
          }
        }
      }
      if (additionalLocales) {
        for (const locale of additionalLocales) {
          if (!(0, generaltranslation_1.isValidLocale)(locale)) {
            throw new Error(
              `locales: "${additionalLocales === null || additionalLocales === void 0 ? void 0 : additionalLocales.join()}", ${locale} is not a valid locale!`
            );
          }
        }
      }
      ...
      const { apiKey, projectId, defaultLocale } = options,
        rest = __rest(options, ['apiKey', 'projectId', 'defaultLocale']);
      if (options.options)
      ...
      if (options.inline) {
        const { updates: newUpdates, errors: newErrors } =
          yield this.createInlineUpdates(options);
      ...
      updates = updates.map((update) => {
        ...
        if (existingHash) {
          if (existingHash !== update.metadata.hash) {
            errors.push(
              `Hashes don't match on two translations with the same id: ${chalk_1.default.blue(update.metadata.id)}. Check your ${chalk_1.default.green(`<T id="${chalk_1.default.blue(update.metadata.id)}">`)} tags and make sure you're not accidentally duplicating IDs.`
            );
      ...
      if (errors.length > 0) {
        if (options.ignoreErrors) {
          console.log(
            chalk_1.default.red(
              `CLI tool encountered errors while scanning for ${chalk_1.default.green('<T>')} tags.\n`
            )
          );
          console.log(
            errors
              .map(
                (error) => chalk_1.default.yellow('• Warning: ') + error + '\n'
              )
              .join(''),
            chalk_1.default.white(
              `These ${chalk_1.default.green('<T>')} components will not be translated.\n`
            )
          );
        } else {
          console.log(
      ...
      if (updates.length) {
        ...
        const globalMetadata = Object.assign(
          Object.assign({}, projectId && { projectId }),
        ...
        const spinner = yield (0, console_1.displayLoadingAnimation)(
        ...
        try {
          ...
          if (options.options)
            (0, updateConfig_1.default)(
              Object.assign(
                { configFilepath: options.options, _versionId: versionId },
                options.locales && { locales: options.locales }
              )
            );
          if (options.enableTimeout && locales) {
            ...
            yield (0, waitForUpdates_1.waitForUpdates)(
              apiKey,
              options.baseUrl,
              versionId,
              locales,
