...
function runJsFallback(reason: string): void {
  console.error(`${reason}\nFalling back to the JS implementation of gt.`);
  import('../main.js').catch((error) => {
    ...
    if (code === 'ERR_MODULE_NOT_FOUND' || code === 'MODULE_NOT_FOUND') {
      ...
      console.error(`gt failed while running the JS fallback: ${message}`);
    }
    process.exit(1);
  });
}
...
function routeToBinary(): void {
  ...
  if (!target) {
    runJsFallback(
      `No prebuilt gt binary exists for your platform (${process.platform}-${process.arch}).`
    );
    return;
  }
