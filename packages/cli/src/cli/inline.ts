  ...
  protected async handleGenerateSourceCommand(
    ...
    if (settings.files && settings.files.placeholderPaths.gt) {
      ...
      for (const locale of settings.locales) {
        // Pseudo-locale files are machine-generated by --pseudo; merging
        // source strings into them would dilute the pseudo content
      ...
      logger.error(
        '--pseudo requires a gt output path in your files configuration (files.gt.output in gt.config.json).'
      );
      exitSync(1);
    }
  }
