...
import { writeOAuthTokens } from '../../../auth/credentialStore.js';
...
describe('gt api', () => {
  ...
  it('sends the signed-in user token when no API key is configured', async () => {
    const stateHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gt-api-auth-'));
    ...
    await writeOAuthTokens(
      {
        ...
        resource: 'https://api.gtx.dev/',
        scope: 'openid',
        tokenType: 'Bearer',
        subject: 'user-1',
      },
      'https://dash.generaltranslation.com/api/auth'
    );
    const fetchMock = vi.fn<typeof fetch>(async (request) => {
