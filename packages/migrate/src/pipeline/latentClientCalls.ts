...
export function detectLatentClientCallHazards(ctx: MigrationContext): void {
  ...
  // lower bound while one exists (round-10 A8). ctx.unreadableFiles is the one
  // owner of that fact; the teardown scan and the report both read it.
  ...
  for (const file of projectFiles) {
    ...
    // never saw, and so could a file we could not read (round-10 A8). Keeping
    // the hazard with no reaching entries makes the emit phase withhold
    // globally instead of shipping a build that crashes on prerender.
    if (
