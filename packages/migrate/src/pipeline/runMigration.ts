...
export async function runMigration(
  ...
  // only as far as EDITS go: returned edits and todos are discarded here and
  // regenerated by the Pass 3 apply against the settled skip set. The calls
  // are NOT pure, though (round-10 arch finding A3): transformNextConfig
  // writes ctx.nextConfigRetainsPlugin, a set-only flag the emit phase reads.
  // That stays correct because 2b-pre classifies against a SUBSET of the
  // skips Pass 3 sees, so a flag set here is set again there; that subset
  // relationship is the monotonicity the assertion after the layout
  // fixed-point loop enforces.
  ...
  // The skip set may only GROW from here (round-10 arch finding A3): 2b-pre's
  // set-only retention flag and the settled-results claim below both rest on
  // that, and nothing enforced it. Snapshot the fixed point; verified before
  // emit.
  ...
  // Skip-set monotonicity, enforced (round-10 arch finding A3): a file
  // unskipped after the layout fixed point would invalidate every retention
  // decision derived from it, silently. This is a bug in gt migrate if it
  // fires, never a project problem.
