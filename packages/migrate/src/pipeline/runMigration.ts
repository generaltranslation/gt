...
import { isLayoutFile } from '../fs/layoutFiles.js';
...
export async function runMigration(
  ...
  if (!options.yes && !options.dryRun) {
    ...
    // CI job reads as a successful no-op migration, so refuse loudly instead
    // (round-10 claims finding 6).
  ...
  // Routed locales and migrated locales must agree, or a prerendered page
  // throws on its first t() over a dictionary that cannot serve it. Every
  // catalog source funnels through here, including the prompt (round-10 A1).
  if (routing.locales) {
  ...
  // only as far as edits go: these edits and todos are discarded and
  // regenerated by Pass 3. transformNextConfig also sets the retention flag
  // the emit phase reads, which Pass 3's larger skip set re-sets (round-10 A3).
  ...
  // The skip set may only grow from here: 2b-pre's set-only retention flag and
  // the settled-results claim below both rest on it (round-10 A3).
  const skipsAtFixedPoint = [...ctx.skippedFiles.keys()];
    ...
    for (const configFile of findRootFiles(cwd, adapter.nextConfigCandidates)) {
      try {
        ...
        if (isEsmNextConfig(configFile, cwd)) {
          // gt-next/config resolves the Next.js version with a bare `require`
          // at module scope and its ESM bundle ships no createRequire shim, so
          // a native-ESM config throws at every version (round-10 finding 4).
          const reason =
  ...
  // Every root-level candidate, not just middleware.*: Next 16 runs
  // proxy.ts/proxy.js the same way (round-10 finding 9). Root-level means no
  // directory part; the src/ forms in the list are already where Next looks.
  ...
  // A file unskipped after the layout fixed point would silently invalidate
  // every retention decision derived from it (round-10 A3). Firing here is a
  // bug in gt migrate, never a project problem.
  for (const file of skipsAtFixedPoint) {
    if (!ctx.skippedFiles.has(file)) {
      io.fatal(
        `internal error (a bug in gt migrate, not your project): ${file} was ` +
          'classified as skipped and later unskipped; retention decisions ' +
          'derived from the skip set cannot be trusted. Nothing has been written.'
      );
    }
  }
  ...
  // collect()'s unchanged-write rule, applied to emitted files: a re-run
  // rewrites gt.config.json and the catalogs byte-identically (round-10 N2).
  // After the key audit, so those catalogs are still in hand above.
...
/**
 * A write whose bytes already match is not a change, and listing it as
 * Converted claims the run touched a file it did not (round-10 N2). Callers
 * differ only in where the current bytes come from: in hand, or read off disk.
 */
