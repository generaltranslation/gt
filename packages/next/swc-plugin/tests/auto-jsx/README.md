...
These 17,049 unique JSX inputs compare **only** automatic JSX insertion with the
...
The CLI and compiler currently agree on 7,198 inputs and differ on 9,851. The
...
`cli-divergences.ts` documents 19 existing difference families, each with a
...
`--serve` retains a dev server for browser inspection.
