use crate::hash::{HtmlContentProps, SanitizedChild, VariableType};
use swc_core::ecma::ast::*;

use crate::whitespace::{has_significant_whitespace, is_normal_whitespace, trim_normal_whitespace};
...
pub fn js_number_to_string(value: f64) -> String {
  ...
  if !(1e-6..1e21).contains(&abs_value) {
...
pub fn filter_jsx_children(children: &[JSXElementChild]) -> Vec<JSXElementChild> {
  ...
  let filtered_children: Vec<JSXElementChild> = filtered_children
    .into_iter()
    .filter(|child| {
...
pub fn build_sanitized_text_content(text: &JSXText) -> Option<SanitizedChild> {
  ...
  if trim_normal_whitespace(&content).is_empty() {
    ...
    if !in_newline_sequence
      && !whitespace_sequence.is_empty()
      && !trim_normal_whitespace(&result).is_empty()
    {
      result.push_str(&whitespace_sequence);
    }
...
mod tests {
  ...
  mod get_variable_type {
    ...
    #[test]
    fn identifies_derive_variable() {
      assert_eq!(get_variable_type("Derive"), VariableType::Static);
    }
    ...
    fn defaults_to_variable() {
      ...
      assert_eq!(
        get_variable_type("SomeCustomComponent"),
