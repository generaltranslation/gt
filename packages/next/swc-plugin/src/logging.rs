            ...
            for (i, current_level) in levels.iter().enumerate() {
                ...
                for (j, test_level) in levels.iter().enumerate() {
                    ...
                    assert_eq!(
                        ...
                        current_level,
                        if should_be_enabled { "enable" } else { "disable" },
                        test_level
                    );
                }
            }
    ...
    mod integration_tests {
        ...
        fn logger_with_different_configurations() {
            let configs = [
                ...
                (LogLevel::Info, vec![LogLevel::Error, LogLevel::Warn, LogLevel::Info]),
                (LogLevel::Debug, vec![LogLevel::Error, LogLevel::Warn, LogLevel::Info, LogLevel::Debug]),
            ];

            for (config_level, enabled_levels) in configs {
                let logger = Logger::new(config_level.clone());
                
                let all_levels = [LogLevel::Error, LogLevel::Warn, LogLevel::Info, LogLevel::Debug];
                
                for level in all_levels.iter() {
                    let should_be_enabled = enabled_levels.iter().any(|enabled| {
                        std::mem::discriminant(enabled) == std::mem::discriminant(level)
                    });
