...
pub fn is_variable_component_name(name: &Atom) -> bool {
  matches!(name.as_ref(), "Var" | "Num" | "Currency" | "DateTime" | "Static" | "Derive")
}
...
mod tests {
  ...
  mod is_variable_component_name {
    ...
    fn recognizes_all_variable_components() {
      let valid_components = ["Var", "Num", "Currency", "DateTime", "Static", "Derive"];
  ...
  mod comprehensive_validation {
    ...
    fn no_overlap_between_categories() {
      let all_names = [
        "T", "Var", "Num", "Currency", "Static", "Derive", "DateTime", "Branch", "Plural", "useGT", "getGT",
      ];
