...
var defaultInitGTProps_1 = __importDefault(
  require('./config/props/defaultInitGTProps')
);
...
function initGT(props) {
  ...
  if (envApiKey) {
    var apiKeyType =
      (_a =
        envApiKey === null || envApiKey === void 0
          ? void 0
          : envApiKey.split('-')) === null || _a === void 0
        ? void 0
        : _a[1];
    if (apiKeyType === 'api') {
      apiKey = envApiKey;
  ...
  var envConfig = __assign(
    __assign(
      __assign({}, projectId ? { projectId: projectId } : {}),
      apiKey ? { apiKey: apiKey } : {}
    ),
    devApiKey ? { devApiKey: devApiKey } : {}
  );
  // ---------- MERGE CONFIGS ---------- //
  // precedence: input > env > config file > defaults
  var mergedConfig = __assign(
    __assign(
      __assign(__assign({}, defaultInitGTProps_1.default), loadedConfig),
      envConfig
    ),
    props
  );
  ...
  if (
    mergedConfig.runtimeUrl === defaultInitGTProps_1.default.runtimeUrl ||
    mergedConfig.cacheUrl === defaultInitGTProps_1.default.cacheUrl
  ) {
    var warningLocales = (
      mergedConfig.locales || defaultInitGTProps_1.default.locales
    ).filter(function (locale) {
      return !(0, supported_locales_1.getSupportedLocale)(locale);
    });
  ...
  return function (nextConfig) {
    ...
    return __assign(__assign({}, nextConfig), {
      env: __assign(__assign({}, nextConfig.env), {
      ...
      webpack: function webpack() {
        ...
        var webpackConfig = _a[0],
          options = _a[1];
        if (resolvedI18NFilePath) {
          webpackConfig.resolve.alias['gt-next/_request'] =
            path_1.default.resolve(webpackConfig.context, resolvedI18NFilePath);
        }
        if (resolvedDictionaryFilePath) {
          webpackConfig.resolve.alias['gt-next/_dictionary'] =
        ...
        if (
          typeof (nextConfig === null || nextConfig === void 0
            ? void 0
...
function resolveConfigFilepath(fileName, cwd) {
  ...
  for (
    var _i = 0,
      _a = __spreadArray(
        __spreadArray([], withExtensions('./'.concat(fileName)), true),
        withExtensions('./src/'.concat(fileName)),
        true
      );
    _i < _a.length;
    _i++
