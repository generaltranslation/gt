...
4. Pass the returned `locale` and `translations` props explicitly to
   `GTProvider` in `pages/_app.tsx`.
5. Remove a hard-coded `lang` prop from a custom `pages/_document.tsx` so
   Next.js can set the active locale on `<html>`.
6. Clear the old GT locale cookie if desired. The migrated Pages Router path
   uses `NEXT_LOCALE` instead; the old cookie remains readable only by the
   compatibility detector when `context.locale` is unavailable.
...
header, the configured legacy GT locale cookie, `Accept-Language`, then the GT
default. Once Next.js i18n routing is configured, `context.locale` is present,
so stale legacy request state cannot override the Next.js route.
...
`GTProvider` does not infer its locale from `router.locale`.

## Locale selection and `NEXT_LOCALE`

The Pages Router `GTProvider`, `LocaleSelector`, and `useSetLocale()` now persist
the selected locale in Next.js's `NEXT_LOCALE` cookie and navigate with:

```ts
Router.push(Router.pathname, Router.asPath, { locale: nextLocale });
```

The cookie is client-readable and uses these attributes:

- `Path=/`
- `Max-Age=31536000` (one year)
- `SameSite=Lax`
- `Secure` on HTTPS, omitted on HTTP development servers

It is intentionally not `HttpOnly`, because the client-side locale selector
writes it. The cookie is host-only. With domain routing, Next.js performs the
explicit locale transition, but a preference written on one domain is not
automatically shared with another domain.
...
With `localeDetection: false`, explicit locale switching still works and the
cookie is still stored, but Next.js will not use the cookie or
`Accept-Language` to redirect an unprefixed root request automatically.

## Remove stale GT locale state

The migrated Pages Router flow does not use `generaltranslation.locale`, a
custom GT locale cookie, the GT locale-reset cookie, or the GT locale-routing
cookies when Next.js supplies `context.locale`. They therefore cannot override
an explicit Next.js locale route or `NEXT_LOCALE`. The old locale cookie is
still considered by the compatibility detector when `context.locale` is
unavailable.
...
already set `NEXT_LOCALE` keep that preference.
