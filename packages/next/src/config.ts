...
type InternalGTConfigProps = BaseWithGTConfigProps &
  ...
  ConfigFileShape & {
    ...
    // _tagIds is declared on BaseWithGTConfigProps (the public props).
  };
...
export function withGTConfig<TNextConfig extends object = NextConfig>(
  ...
  const swcPluginOptions: Record<string, unknown> = {
    ...
    ...(jsxImportSourceFromLoader && {
      ...
      missingJsxRuntimeContextDiagnostic: createGtNextPluginDiagnostic({
        severity: 'Error',
        whatHappened: 'The JSX runtime context is missing from this module',
        why: 'Automatic JSX injection with Emotion needs the current React server condition',
        fix: 'Keep the loaders and SWC options generated by withGTConfig together, then restart Next.js',
      }),
    }),
  };
  ...
  const config: NextConfig = {
    ...
    ...(turboPackEnabled &&
      !experimentalTurbopack && {
        turbopack: {
          ...
          ...(jsxImportSourceFromLoader && {
            rules: addAutoJsxLayerLoader(
              internalNextConfig.turbopack?.rules,
              autoJsxLayerLoader
            ),
          }),
          resolveAlias: {
      ...
      ...(turboPackEnabled &&
        experimentalTurbopack && {
          turbo: {
            ...
            ...(jsxImportSourceFromLoader && {
              rules: addAutoJsxLayerLoader(
                internalNextConfig.experimental?.turbo?.rules,
                autoJsxLayerLoader
              ),
            }),
            resolveAlias: {
    ...
    webpack: function webpack(
      ...
      if (!turboPackEnabled) {
        if (jsxImportSourceFromLoader) {
          webpackConfig.plugins ??= [];
          webpackConfig.plugins.push(
            new AutoJsxWebpackPlugin(resolvedWasmFilePath)
          );
        }
