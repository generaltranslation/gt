...
The `downloadFileBatch()` method downloads multiple translation files in a single batch request, providing better performance and efficiency when retrieving many files compared to individual download calls. The files are returned with their content decoded from Base64.
