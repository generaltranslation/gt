...
`useDict()` is used to access string translations from the [translation dictionary](/docs/react/guides/dictionaries).
...
<Callout>
  `useDict()` uses a [dictionary](/docs/react/guides/dictionaries) to store all content for translation.
