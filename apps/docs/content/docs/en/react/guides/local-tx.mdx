...
Pass `loadTranslations` as a prop to the [`<GTProvider>`](/docs/react/api/config/provider) component:
...
- API References:
  - [`loadTranslations()` Function](/docs/react/api/config/load-translations)