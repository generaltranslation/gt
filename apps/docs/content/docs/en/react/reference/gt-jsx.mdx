...
```ts
["The count is ", { k: "_gt_num_1", v: "n", i: 1 }]
```
...
To avoid computing hashes at runtime, the libraries allow the user to specify an ID to check before calculating the hash.
...
If the ID is present in the translation JSON file, the corresponding JSX tree will be used and the hash will never be computed.
