...
### SSG with custom getLocale [#ssg-custom-get-locale]

SSG can disrupt locale resolution.
Because of this, you will have to define your own `getLocale()` function that accesses the locale through the url path.
Depending on your version of Next.js, your solution may be different.

#### Next.js 15.5+

Create a new file in the root of your project called `getLocale.ts`.

```tsx title="getLocale.ts"
...
Unfortunately, for earlier versions there is no way to access the url path parameters with SSG.
You will have to upgrade to Next.js 15.1 or later.
