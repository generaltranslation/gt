...
The `<Branch>` and `<Plural>` components should be used within a [`<T>`](/docs/next/guides/jsx) component to sanitize dynamic content for translation.
...
<Callout>
  ...
  However, they are often used within a [`<T>`](/docs/next/guides/jsx) component to sanitize dynamic content for translation.
</Callout>
