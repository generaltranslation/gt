...
This guide will walk you through how to easily internationalize shared strings in your Next.js app using the [`msg()`](/docs/next/api/strings/msg) function & [`useTranslations()`](/docs/next/api/dictionary/useTranslations) hook.
...
If you are in an async component, use the `getTranslations()` function instead of the `useTranslations()` hook.
...
<Callout type='warn'>
  ...
  If want to get back the original string, you need to decode it with [`decodeMsg()`](/docs/next/api/strings/msg#decodemsg)
</Callout>
...
For more information on the API, see the [API reference](/docs/next/api/strings/msg).
...
## Next Steps
 * See the [API reference for `msg()`](/docs/next/api/strings/msg).
 * See the [API reference for `useTranslations()`](/docs/next/api/dictionary/useTranslations).
