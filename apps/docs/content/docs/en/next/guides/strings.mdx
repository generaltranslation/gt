...
String translation provides direct access to text translations without JSX, perfect for attributes, object properties, and plain text values. Use [`useGT()`](/docs/next/api/strings/useGT) in client components and [`getGT()`](/docs/next/api/strings/getGT) in server components.

## Client vs Server Usage

- **Client Components**: [`useGT()`](/docs/next/api/strings/useGT) hook for React components
- **Server Components**: [`getGT()`](/docs/next/api/strings/getGT) async function for server-side rendering
...
```jsx
// ❌ Wrong: useGT in server component
export default async function ServerComponent() {
  const t = useGT(); // This won't work
  return <p>{t('Hello')}</p>;
}

// ✅ Correct: getGT in server component  
export default async function ServerComponent() {
  const t = await getGT();
  return <p>{t('Hello')}</p>;
}

// ✅ Correct: useGT in client component
'use client';
