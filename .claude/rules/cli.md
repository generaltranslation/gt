...
- CLI entry point is `src/main.ts`. Library entry point is `src/index.ts`. Put command/runtime logic under `src` subdirectories rather than adding non-entrypoint files at the `src` root.
