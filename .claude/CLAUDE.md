...
- Format new user-facing logs, warnings, errors, thrown error messages, and validation messages that report actionable problems with `createDiagnosticMessage()`. Import it from `@generaltranslation/utils`; `generaltranslation/internal` remains a compatibility path, and code inside `packages/core` can import the local re-export from `src/logging/diagnostics.ts`. Routine progress, status, and debug output does not need to be a diagnostic.
