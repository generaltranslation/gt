...
This guide will walk you through how to internationalize strings in your React app using the `useGT()` hook.
...
In most cases, you can use the [`<T>` component](/docs/react/guides/jsx).
...
<Callout>
  Wherever possible, you should use the [`<T>` component](/docs/react/guides/jsx).
...
The `useGT()` hook has to be called within the [`<GTProvider>`](/docs/react/api/components/gtprovider).
...
For more information on the API, see the [API reference](/docs/react/api/strings/useGT).
...
<Accordions>
  <Accordion title="Why can't I translate dynamic content?">
    This is because React is client-side rendered, and no translations can ever occur in production
    due to security vulnerabilities involving exposed API keys. 
    In development, translations will work because the development server
 ...
 * See the [API reference for `useGT()`](/docs/react/api/strings/useGT).