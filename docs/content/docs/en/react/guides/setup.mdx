...
<Callout type="info">
  Depending on your framework, you may need to prefix the environment variables, for example `NEXT_PUBLIC_GT_API_KEY` or `VITE_GT_API_KEY`.
...
You can also use the CLI tool command `npx gtx-cli auth` to generate an API key and project ID for your project, saved to your `.env.local` file.
