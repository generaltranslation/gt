...
The ESM build sets `polyfillRequire: false` so rolldown does not inject a package-wide `createRequire`-from-`node:module` shim, uses an explicit server-only `createRequire(import.meta.url)` for custom request function loading, and marks the `index.rsc`/`index.client` entrypoints as having side effects so their top-level initialization is not tree-shaken away.
