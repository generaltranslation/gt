...
withGTConfig's webpack hook now parses gt-next's ESM dist as javascript/auto whenever one of these aliases is configured, so webpack picks up the require() calls and the aliases apply. Dist output is unchanged and turbopack behavior is untouched.
