...
Apply the same locale conversion in the CLI and Sanity API adapters: resolve custom aliases, then standardize language-code spelling for upload, download, enqueue, file-query, and project-setup requests. Preserve canonicalization for CLI project creation and user-edit requests and Sanity single-file downloads. Lowercase configured locale identities such as `en-us` are sent as `en-US`. Custom mapping `code` values must use canonical spelling, for example `'en-gb': { code: 'en-GB' }`.
