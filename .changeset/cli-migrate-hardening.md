...
- The on-demand engine fetch works on Windows: npm is npm.cmd there, which spawnSync cannot execute directly, so the install now runs through the shell on win32 with the version range and cache path quoted (the caret in `^0.1.0` is a cmd.exe escape character). Previously every Windows first run failed over to the manual devDependency instruction.
- The react-intl and react-i18next source-locale fallbacks reference the canonical `libraryDefaultLocale` constant instead of repeating the `'en'` literal, matching the next-intl lane and the repo-wide canonical-defaults check. Same value, so no behavior change.

Fifth review round:

- A `localePrefix` or `pathnames` carried only by an unresolvable spread in the routing config (`defineRouting({ ...base })`) is now reported unresolved instead of absent, so the middleware no longer assumes next-intl's default `localePrefix: 'always'` (which would prefix the default locale) and navigation no longer proceeds as if there were no localized pathnames. Both transforms skip the affected file with a report entry, unless a literal property after the spread resolves the field.
- An existing `files.gt` entry in `gt.config.json` is preserved instead of being overwritten with the migration's default output. A hybrid or partially migrated project keeps its custom `output`/`parsingFlags`/`publish`, and the report notes the preserved entry so it can be verified. A project with no `files.gt` still gets the default `public/_gt/[locale].json`.
- The global library detector (used by pre-existing commands like JSON aggregation) no longer selects `react-intl`. A project with both `react-intl` and `i18next` installed keeps its `i18next` JSON aggregation format instead of falling through to `STRING`. `gt migrate` is unaffected because it selects its source from the required `--from` flag.
- Actionable `gt migrate` failures and warnings in the command and the pipeline now go through `createDiagnosticMessage` (via a small migration-specific wrapper in the engine) instead of hand-assembled strings, so they carry the repository's consistent source, severity, and recovery formatting. Routine progress output stays plain.
- Spread hardening and diagnostic-prefix cleanup: an inline object-literal spread in the routing config (`defineRouting({ ...{ localePrefix: 'as-needed', pathnames } })`) is now treated as unresolvable like a variable spread, so the field is reported unresolved instead of silently absent and the middleware/navigation transforms skip rather than emit next-intl's default prefixing or drop localized pathnames. The remaining catalog-discovery and interactive-fallback diagnostics also route through the wrapper, so every actionable migrate message carries the same `gt` prefix regardless of source library.

Close two defects from monorepo field testing (2026-07-23 review round):

- Side-effect imports survive migration: a pre-existing zero-specifier import (`import './globals.css'`) is never removed by the unused-import cleanup that runs on mutated layouts, sources, and middleware. Side-effect imports of the source library itself (`import 'next-intl'`, `@formatjs/*` polyfills) now hold the file with an explicit skip reason instead of being deleted silently; bare `i18next` imports already skipped as bespoke setup.
- Workspace-aware install: when the migrated app is a monorepo workspace member with the lockfile at the repo root, `gt migrate` resolves the nearest workspace root (lockfile plus `workspaces` field or `pnpm-workspace.yaml`) instead of failing leaf-only detection and exiting without installing. npm installs run at the root with `--workspace=<member>` so no second dependency tree is created; pnpm, yarn, and bun install from the member as before.
